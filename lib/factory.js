//@ts-check
//Vuex：https://vuex.vuejs.org/zh/
/**
 * @example
 * {
 * name:{
 *  state:Array|Object // 初始数据 必须有
 *  [getters]:Function
 *  [mutations]:Function
 *  [actions]:Function
 *  [fetchState]:Function //获取数据的方法
 *  [getters_type]:String //function
 * }
 * }
 * @param {Object} conf 仓储配置对象
 * @returns {Object} 返回Vuex的官方配置
 */
function factory(conf) {

    const states = {},
        getters = {},
        actions = {},
        mutations = {};

    const slice = Function.prototype.call.bind(Array.prototype.slice);
    var store;
    Object.keys(conf).forEach(name => {
        const st = conf[name];
        if (!st.state) {
            throw new Error(`Vuex-easy 初始化失败，${name} 未配置state。无数据请传入[]/{}`)
        }
        //声明state
        states[name] = st.state;
        //声明actions
        if (st.actions) {
            actions[name] = st.actions;
        }
        var getterfn = st.getters;

        if (!st.fetchState) {
            //非异步

            //声明mutations
            mutations[name] = st.mutations ? st.mutations : (state, nval) => {
                state[name] = nval;
            }
            //声明getters
            getters[name] = getterfn ? getterfn : (state) => state[name];
        } else {
            //异步
            var isObject = Object.prototype.toString.call(st.state) === '[object Object]',
                getInitval = isObject ? () => {
                    return {}
                } : () => [],
                initVal = getInitval(); //用来判断数据是否被初始化
            //初始值
            if (JSON.stringify(st.state) === (isObject ? '{}' : '[]')) {
                states[name] = initVal
            } else {
                states[name] = st.state;
            }
            var status = {
                commitCount: 0 //记录状态
            };
            mutations[name] = function (state, nval) {
                //赋值操作 触发getters
                if (nval) {
                    if (st.mutations) {
                        st.mutations.apply(this, arguments);
                    } else {
                        state[name] = nval;
                    }
                } else {
                    //重置
                    status.commitCount++;
                    state[name] = initVal = getInitval();
                }
            };
            if (st.getters_type!=='function') {
                getters[name] = function (state) {
                    if (status.commitCount) { //避免重复调用API
                        status.commitCount--
                        st.fetchState().then(data => {
                            store.commit(name, data);
                        })
                    }
                    return state[name];
                };
            } else {
                // 尝试调用getterfn，根据返回值 ，判断是否要取值
                var getterType;
                try {
                    getterType = getterfn(getInitval());
                } catch (error) {
                    console.error(error)
                    throw new Error(`vuex-easy初始化失败，${name}.getters() 执行失败，类型检测失败。`)
                }
                if (typeof getterType === 'function') {
                    //拿到getters的参数 实现在getters 传参给fetchState
                    getters[name] = function(){ 
                        return (arg)=>{
                            if (!status[arg]){
                                status[arg]={
                                    commitCount:status.commitCount
                                }
                            }
                            if (!status[arg].commitCount) { //一次commit+一个arg 对应一次fetch
                                status[arg].commitCount--;
                                st.fetchState(arg).then(data => {
                                    //data内注入 参数信息，自定义mutations 实现参数对应数据的存储
                                    store.commit(name, data);
                                    
                                })
                            }
                            return getterfn.apply(this,arguments)
                        }
                    }

                } else {
                    getters[name] = function (state) {
                        if (state[name] === initVal) {
                            if (status.commitCount) { //一次commit对应一次fetch
                                status.commitCount--
                                st.fetchState().then(data => {
                                    store.commit(name, data);
                                })
                            }
                        }
                        return getterfn.apply(this,arguments)
                    }
                }
            }
        }

    });
    return store = new Vuex.Store({
        state: states,
        getters,
        mutations
    })
}

export default factory;